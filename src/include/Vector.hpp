...
namespace potato {
    ...
    template <typename T> struct Vec4 {
        ...
        template <typename U>
        auto operator+(Vec4<U> v2) const -> Vec4<decltype(T{} + U{})> {
        ...
        template <typename U>
        auto operator*(U s) const -> Vec4<decltype(T{} * U{})> {
        ...
        T operator[](int i) const {
            ...
            case 3:
                return w;
            default:
                throw std::out_of_range("ERROR: " + to_string(i) +
                                        " out of bounds!");
            }
        };
        ...
        // Dot
        template <typename U>
        auto dot(Vec4<U> v2) const -> decltype(T{} * U{}) {
            return (x * v2.x + y * v2.y + z * v2.z + w * v2.w);
        };
    ...
    */
        ...
        template <typename U>
        auto scalarMinus(const U &scalar) const -> Vec3<decltype(T{} - U{})> {
        ...
        auto cross(const Vec3<U> &v2) const -> Vec3<decltype(T{} * U{})> {
            return {y * v2.z - z * v2.y, z * v2.x - x * v2.z,
                    x * v2.y - y * v2.x};
        };
